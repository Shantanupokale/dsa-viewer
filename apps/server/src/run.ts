...
/** Language -> sandbox image. Fixed map — never derived from user input. */
const IMAGES: Partial<Record<Language, string>> = {
  go: "dsa-run-go:0.1.0",
};

/** go.mod for the job: module `run`, tracer resolved via the image's vendored copy. */
...
export async function runCode(req: RunInput, config: Config): Promise<RunResult> {
  const image = IMAGES[req.language];
  if (!image) {
    return {
      ...
      stdout: "",
      durationMs: 0,
      error: `Language "${req.language}" is not supported yet (Phase 0 is Go-only).`,
    };
  }

  const sandbox = await runInSandbox({
    image,
    files: { "main.go": req.code, "go.mod": GO_MOD, "input.txt": req.input },
